...
function PracticeTimer({ onStartFullScreen, onUpdateTimer, isRunning: externalIsRunning }) {
  const [practiceType, setPracticeType] = useState('numerical') // 'numerical' or 'verbal'
  const [hours, setHours] = useState(1)
  const [minutes, setMinutes] = useState(45)
  const [isRunning, setIsRunning] = useState(false)
  const [timeLeft, setTimeLeft] = useState(null)
  const [displayTime, setDisplayTime] = useState('01:45:00')
  const intervalRef = useRef(null)

  // Deneme türüne göre varsayılan süreler
  useEffect(() => {
    if (practiceType === 'numerical') {
      setHours(1)
      setMinutes(45)
    } else {
      setHours(1)
  ...
  const handleAlarm = () => {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
      new Notification('Deneme Süresi Doldu!', {
        body: `${practiceType === 'numerical' ? 'Sayısal' : 'Sözel'} deneme süreniz tamamlandı.`,
        icon: '/icon.png'
      })
      ...
      <div className="flex flex-col items-stretch justify-start">
        <div className="flex w-full grow flex-col items-stretch justify-center gap-2">
          ...
          <p className="text-[#9cabba] text-base font-normal leading-normal">
            Deneme türünü seçin ve geri sayımı başlatın.
          </p>
        </div>
      </div>
      ...
      {isRunning && timeLeft !== null ? (
        <div className="flex w-full flex-col mt-6">
          <div className="my-6 flex flex-col items-center justify-center rounded-lg bg-black/20 py-8">
            <span className="text-6xl font-bold tracking-tighter text-[#48D1CC] md:text-8xl">
              {displayTime}
            </span>
            <div className="mt-4">
              <span className="text-[#9cabba] text-sm font-medium">
                {practiceType === 'numerical' ? 'Sayısal' : 'Sözel'} Deneme
              </span>
            </div>
          </div>
          <div className="flex justify-stretch pt-2">
            <div className="flex flex-1 flex-wrap gap-3 justify-start">
              <button
                onClick={handleStart}
                className="flex min-w-[84px] flex-1 cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-lg bg-[#48D1CC] px-4 text-sm font-bold leading-normal tracking-[0.015em] text-background-dark h-12"
              >
        ...
        <div className="flex w-full flex-col mt-6">
          <div className="flex w-full items-end gap-4 pb-4">
            <div className="grid w-full grid-cols-2 gap-3 rounded-lg bg-[#283039] p-1.5">
              <button
                onClick={() => setPracticeType('numerical')}
                className={`flex min-w-[84px] cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-md px-4 text-sm font-bold leading-normal tracking-[0.015em] h-11 transition-colors ${
                  practiceType === 'numerical'
                    ? 'bg-[#48D1CC] text-background-dark'
                    : 'text-white/60 hover:bg-white/5'
                }`}
              >
                <span className="truncate">Sayısal</span>
              </button>
              <button
              ...
              >
                <span className="truncate">Sözel</span>
              </button>
            </div>
          </div>
          <div className="flex w-full flex-wrap items-end gap-4 pb-4">
            <label className="flex flex-col min-w-40 flex-1">
              ...
              <input
                className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg border border-[#3b4754] bg-[#1b2127] p-[15px] text-base font-normal leading-normal text-white placeholder:text-[#9cabba] focus:border-[#48D1CC] focus:outline-0 focus:ring-0 h-14"
                placeholder="01"
                type="number"
                min="0"
                max="23"
          ...
          <div className="flex justify-stretch pt-2">
            <div className="flex flex-1 flex-wrap gap-3 justify-start">
              ...
              >
                ...
                <span className="truncate">Denemeyi Başlat</span>
              </button>
              <button
                onClick={handleReset}
                className="flex min-w-[84px] cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-lg bg-[#283039] px-4 text-sm font-bold leading-normal tracking-[0.015em] text-white h-12"
